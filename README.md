...
The branch `workshop` contains incremental commits, showing building blocks one by one. Start at the commit that says
 "Initial commit for the Workshop". How? Go to the 
 [list of commits for that branch here](https://github.com/ividal/TransferLearning/commits/workshop), look for that 
 commit message, copy it's `SHA` and do `git checkout $SHA`).
